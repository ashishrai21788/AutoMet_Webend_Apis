...
const DriverAppAnalytics = require('../models/driverAppAnalyticsModel');
...
/**
 * Save driver app analytics events to MongoDB (collection: driver_app_analytics)
...
exports.saveAnalytics = async (req, res) => {
  try {
    ...
    const source = body.source ?? 'driver_app';
    ...
    const normalizedEvents = events.map((evt) => {
      ...
      event.eventCategory = (raw.eventCategory && String(raw.eventCategory).trim()) ? String(raw.eventCategory).trim() : 'driver';
      event.actorId = raw.actorId ?? raw.actor_id ?? null;
      event.actorType = raw.actorType ?? raw.actor_type ?? 'driver';
      event.pageIdentifier = raw.pageIdentifier ?? raw.page_identifier ?? null;
      event.metadata = raw.metadata ?? {};
      // appId, deviceId, sessionId, source, platform, appVersion: document level only (not duplicated per event)
      if (event.params && event.params.onlineStartedAt) {
        event.params = { ...event.params, onlineStartedAt: new Date(event.params.onlineStartedAt) };
      }
      return event;
    });
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[driver-app-analytics] Saved', saved._id, 'events:', saved.events.length);
    }

    return res.status(201).json({
      success: true,
      message: 'Driver analytics events saved successfully',
      data: {
        id: saved._id,
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('[driver-app-analytics] Save error:', error.message, error.name);
    }
